...
public interface MovieCinemaRepository extends JpaRepository<MovieCinema, Long> {
    ...
    Integer countAllByCinema_Id(Long cinemaId);

    //Write a derived query to count all movie cinemas with a specific movie id
    Integer countAllByMovie_Id(Long movieId);
    ...
    List<MovieCinema> getAllByMovie_NameContaining(String movieName); // is it true

    //Write a derived query to list all movie cinemas in a specific location name
    List<MovieCinema> getAllByCinema_Location_Name(String locationName);

    // ------------------- JPQL QUERIES ------------------- //

    //Write a JPQL query to list all movie cinemas with higher than a specific date
    @Query("select mc from MovieCinema mc where mc.dateTime > ?1")
    List<MovieCinema> retrieveAllByDateTimeAfter(LocalDateTime dateTime);
